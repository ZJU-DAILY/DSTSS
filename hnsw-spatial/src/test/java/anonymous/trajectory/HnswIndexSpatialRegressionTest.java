...
class HnswIndexSpatialRegressionTest {
  ...
  void setUp() throws IOException {
    index = HnswIndexSpatial2
        .newBuilder(dimensions, distanceFunction, distanceFunction, differenceCalculator, maxItemCount)
  ...
  void addAllCallsProgressListener() throws InterruptedException {
    ...
    assertThat(updates, is(Arrays.asList(
        ...
        new ProgressUpdate(3, 3)  // emitted because it's the last element
    )));
  }
  ...
  void saveAndLoadIndex() throws IOException {
    ...
    HnswIndexSpatial2<String, float[], TestItem, Float> loadedIndex =
        HnswIndexSpatial2.load(new ByteArrayInputStream(in.toByteArray()));
